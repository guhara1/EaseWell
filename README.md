...
`netlify.toml`에 빌드 설정이 들어 있습니다: `command = node build.mjs`, `publish = dist`.

- 도메인: `https://easewell-massage.netlify.app` (`data/site.json`의 `baseUrl`과 일치)
- 루트 `/` → `/gyeonggi-south/` 301 리다이렉트, `dist/404.html` 404 페이지 포함
- **Netlify에서 이 저장소를 연결**하고, **Production branch를 배포할 브랜치로 지정**하면 자동 빌드·게시됩니다.
  (Netlify는 기본적으로 default 브랜치를 게시하므로, 이 브랜치가 default가 아니면
  Site configuration → Build & deploy → Branches 에서 production branch를 지정해야 합니다.)
...
| `heroImage` | `""` (비어있음) | 메인 히어로 배경 이미지 경로. 예: `/assets/hero.jpg` |

### 히어로 이미지 넣는 법

1. 이미지 파일을 저장소 `public/assets/` 에 넣습니다. 예: `public/assets/hero.jpg`
2. 빌드하면 `public/` 폴더가 사이트 루트로 복사되어 `https://easewell-massage.netlify.app/assets/hero.jpg` 로 서빙됩니다.
3. `data/site.json` 의 `"heroImage"` 값을 `"/assets/hero.jpg"` 로 설정 후 `npm run build`.
   → 히어로에 잉크 오버레이가 얹힌 배경 이미지가 적용됩니다(글자 가독성 유지). 비워두면 기존 그라디언트가 유지됩니다.
